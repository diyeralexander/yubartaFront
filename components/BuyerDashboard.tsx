...
const OfferActionModal = ({ offer, isOpen, onClose, onApprove, onReject, currentUser }: any) => {
    ...
    const requirement = useMemo(() => requirements.find(r => r.id === offer.requirementId), [requirements, offer.requirementId]);
    const seller = useMemo(() => users.find(u => u.id === offer.sellerId), [users, offer.sellerId]);
        ...
        <div className="fixed inset-0 bg-[#0A0A0A]/40 backdrop-blur-sm z-50 flex justify-center items-start pt-10">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-[#D6D6D6] overflow-hidden animate-fade-in-up">
                ...
                <div className="p-8 overflow-y-auto">
                    {/* Buyer must NOT see Seller's penalty agreement details here */}
...
const BuyerDashboard = ({ currentUser }: BuyerDashboardProps) => {
  const { requirements, offers, commitments, users, setOffers, setCommitments, setRequirements, setUsers, generateNewRequirementId } = useData();
  ...
  // Handlers
  const handleCreateReq = useCallback((data: any) => {
      ...
      setIsFormOpen(false);
  }, [currentUser.id, generateNewRequirementId, setRequirements]);

  const handleUpdateReq = useCallback((data: any) => {
      if(!requirementToEdit) return;
      setRequirements(prev => prev.map(r => r.id === requirementToEdit.id ? { ...r, ...data, status: RequirementStatus.ACTIVE } : r));
      setIsFormOpen(false);
      setRequirementToEdit(null);
  }, [requirementToEdit, setRequirements]);

  const handleApproveOffer = useCallback((offerId: string) => {
      const offer = offers.find(o => o.id === offerId);
      if(!offer) return;
      const req = requirements.find(r => r.id === offer.requirementId);
      if(!req) return;

      const newCommitment: Commitment = { id: `com${Date.now()}`, offerId: offer.id, requirementId: offer.requirementId, volume: offer.cantidadOfertada };
      setOffers(prev => prev.map(o => o.id === offerId ? { ...o, status: OfferStatus.APPROVED } : o));
      ...
      // We need to include the new commitment in the calculation (it's not in state yet for this calculation)
      const currentVol = allReqCommitments.reduce((s,c) => s + c.volume, 0) + offer.cantidadOfertada;
      
      if (currentVol >= req.totalVolume) {
          setRequirements(prev => prev.map(r => r.id === req.id ? { ...r, status: RequirementStatus.COMPLETED } : r));
      }
  }, [offers, requirements, commitments, setOffers, setCommitments, setRequirements]);

  const handleRejectOffer = useCallback((offer: Offer) => {
      ...
      setOffers(prev => prev.map(o => o.id === offer.id ? { ...o, status: OfferStatus.REJECTED, communicationLog: [...(o.communicationLog || []), newLog] } : o));
  }, [currentUser.id, setOffers]);
                ...
                <Table>
                    ...
                    <tbody>
                        {myRequirements.map(req => {
                            ...
                            // Using local progress calculation for list view, duplicate of ProgressBar logic for simpler list render if needed, or use Component
                            ...
                            return (
                                <TableRow key={req.id} onClick={() => handleReqClick(req.id)}>
                                    <TableCell className="text-[#7A7A7A] font-mono text-xs">#{req.id.slice(-6)}</TableCell>
                                    <TableCell>
                                      ...
                                      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
                                          <div>
                                              <div className="flex items-center gap-2 mb-1">
                                                  ...
                                                  <span className="text-xs text-[#7A7A7A] font-mono">ID: {req.id.slice(-6)}</span>
                                              </div>
  ...
  return (
    <div className="pb-12">
        ...
        <OfferActionModal offer={offerToReview} isOpen={!!offerToReview} onClose={() => setOfferToReview(null)} onApprove={handleApproveOffer} onReject={handleRejectOffer} currentUser={currentUser} />
    </div>
  );
};
